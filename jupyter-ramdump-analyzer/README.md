...
**예시 1. alloc magic 손상 원인 추적**

```
%%ai openrouter/nvidia/nemotron-3-super-120b-a12b -f markdown
커널 메모리 덤프에서 'alloc magic is broken at %p: %lx' 문자열이 발견됐고
동시에 kernel_oops:BUG: 패턴도 검출됐어.
Linux 6.5.0-41-generic 환경에서 이 두 신호가 함께 나타날 때
가장 가능성 높은 root cause와 확인해야 할 커널 함수를 알려줘.
```

**예시 2. OOM + 커널 패닉 연관성 분석**

```
%%ai openrouter/nvidia/nemotron-3-super-120b-a12b -f markdown
덤프에서 'out of memory' 문자열과 grub_malloc/grub_realloc/grub_calloc 패턴,
그리고 crashes:crash 신호가 동시에 검출됐어.
OOM killer가 개입했을 가능성과, grub 관련 할당자가 커널 패닉과 연결되는
시나리오를 단계별로 설명해줘.
```

**예시 3. 외부 IP 17개 트래픽 평가 (변수 보간 활용)**
...
```
%%ai openrouter/nvidia/nemotron-3-super-120b-a12b -f markdown
아래 네트워크 정보를 보고 공격자 침투 또는 데이터 유출 가능성을 평가해줘.
특히 Telnet(23), FTP-data(20) 포트가 활성화된 이유와 위험도를 설명해줘.
...
### Chat UI (`@Jupyternaut`) 예시

**예시 4. Telnet 포트 보안 위협 심층 분석**

```
@Jupyternaut 이 덤프의 메모리에서 Telnet(포트 23)과 FTP-data(포트 20)가
동시에 활성 상태였어. Linux 6.5.0 서버에서 이 두 포트가 열려있을 때
공격자가 취할 수 있는 구체적인 공격 시나리오를 알려줘.
추가로 확인해야 할 프로세스나 로그 위치도 알려줘.
```

**예시 5. woreilly 계정 행위 분석 및 조사 방향**

```
@Jupyternaut 덤프에서 유일한 사용자 계정이 woreilly 하나야.
외부 IP 17개와 통신 흔적이 있고 커널 오류도 발생했어.
이 계정이 공격자 계정이거나 탈취됐을 가능성을 평가하고,
메모리 덤프에서 추가로 확인할 수 있는 아티팩트(bash 히스토리 잔재,
열린 파일 디스크립터, 소켓 상태 등)의 확인 방법을 알려줘.
```
...
```python
# 노트북에서 %%ai로 초기 분석 → 결과를 변수에 저장
initial_hypothesis = "OOM killer 트리거 가능성 높음, grub 할당자 손상 의심"

# Chat UI에서 이어받아 심층 질의
# @Jupyternaut 위 가설을 바탕으로 memory.vmem에서 OOM killer 로그를 찾으려면
# 어떤 커널 심볼이나 문자열 패턴을 추가로 검색해야 해?
```
